...
const Home = () => {
    ...
    useEffect(() => {
        const timeoutId = setTimeout(() => {   // The moment a user types a letter, useEffect starts a silent digital stopwatch for 400ms
            fetchEvents();    // If the user stops typing and the clock successfully ticks down to 400ms, it triggers the internal block: fetchEvents(), which reaches out to your backend.
        }, 400); // 400ms debounce
        return () => clearTimeout(timeoutId);
    }, [search]);    // [search]: This dependency array tells React: "Keep your eyes glued to the search state variable. Every time the user types or deletes a letter, run the instructions inside this block immediately."
